...
TCountry TCountry::gcountries_int[] = 
{
...
int TCountry::InitWithFile(const char * strFile)
{
	...
	if (opened == 0 || g_countries_var_count == 0)
	{
		//
		// init from build-in list
		//
		j = GetCountryCountInt();
		for(i = 0; i < j; i++)
		{
			gcountries_var[i] = gcountries_int[i];
		}
		g_countries_var_count = j;
...
int TCountry::GetCountryCountInt()
{
	return sizeof(gcountries_int) / sizeof(TCountry);
}
