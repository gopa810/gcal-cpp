...
/*
   ...
   SUN - VSOP87 Series Version E
   BARYCENTRIC DYNAMICAL ECLIPTIC AND EQUINOX J2000
...
GCHorizontalCoords sunPosition(int year, int month, int day, int hour=12, int min=0, int sec=0,
                    double lat=48.14455, double longi=17.10775136947632)
{
    ...
    double leap = floor(delta / 4); // former leapyears
...
double SunLong2(double jd)
{
	GCHorizontalCoords cd;
	double longi = 0;
	double lati = 0;
	...
	lati = 17.2;
	longi = 47.8;
	cd = sunPosition(2015, 6, 24, 14, 42, 0, lati, longi);
	TRACE2("1 %f   %f       ", lati, longi);
	TRACE2("%f, %f\n", cd.elevation, cd.azimut);
	lati = -3;
	cd = sunPosition(2012, 12, 22, 12, 0, 0, lati, longi);
	TRACE2("2 %f   %f       ", lati, longi);
	TRACE2("%f, %f\n", cd.elevation, cd.azimut);
	lati = 3;
	cd = sunPosition(2012, 12, 22, 12, 0, 0, lati, longi);
	TRACE2("3 %f   %f       ", lati, longi);
	TRACE2("%f, %f\n", cd.elevation, cd.azimut);
	lati = 41;
	cd = sunPosition(2012, 12, 22, 12, 0, 0, lati, longi);
	TRACE2("4 %f   %f       ", lati, longi);
	TRACE2("%f, %f\n", cd.elevation, cd.azimut);
	TRACE0("- - - - - - - - - - --------------\n");
