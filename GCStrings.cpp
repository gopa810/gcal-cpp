...
NSMutableArray<TString> GCStrings::gstr;
...
{
	return gstr.ObjectAtIndex(i)->c_str();
}
...
int GCStrings::readFile(const char * pszFile)
{
	...
	{
		TFile::CreateFileFromResource(IDR_FILE_STRINGS, pszFile);
	}
	...
	if (rf.Open(pszFile, "rt") != 0)
	{
		...
		while(rf.ReadLine())
		{
			if (atoi(rf.GetTag())==78)
			{
				...
				TString * p = gstr.ObjectAtIndex(index);
				if (p == NULL)
				{
					p = new TString();
					gstr.SetObjectAtIndex(index, p);
				}
	...
	p = new TString();
	p->Format("GCal %s", version);
	gstr.SetObjectAtIndex(131, p);

	p = new TString();
	p->Format("Gaurabda Calendar %s", version);
	gstr.SetObjectAtIndex(132, p);
...
const char * GCStrings::getLongitudeDirectionName(double d)
{
	if (d < 0.0)
		return "West";
...
const char * GCStrings::getLatitudeDirectionName(double d)
{
	if (d < 0.0)
		return "South";
