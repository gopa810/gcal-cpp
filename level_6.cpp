...
void FormatTodayInfoRtf(VCTIME vc, CLocationRef & loc, TString &str)
{
	...
	str2.Format("\\par\\f2\\fs%d {\\fs%d %s}\\line %s (%s, %s, Timezone: %s)\\par\r\n\\par\r\n  %s, %s %s\\par\r\n  %s %s, %d Gaurabda\\par\r\n\\par\r\n",
		g_TextSize, g_TextSize+4, GCStrings::getString(p->date.dayOfWeek).c_str(),
		loc.m_strName.c_str(), EARTHDATA::GetTextLatitude(loc.m_fLatitude), EARTHDATA::GetTextLongitude(loc.m_fLongitude),
		TTimeZone::GetTimeZoneName(loc.m_nDST),
		GCStrings::GetTithiName(p->astrodata.nTithi), GCStrings::GetPaksaName(p->astrodata.nPaksa), GCStrings::getString(20).c_str(), 
		GCStrings::GetMasaName(p->astrodata.nMasa), GCStrings::getString(22).c_str(), p->astrodata.nGaurabdaYear);
...
//	nMin = int((vc.shour*24 - nHour)*60);
	...
	AddTextLine(strText, GCStrings::getString(25));
	AddTextLine(strText, "");
	...
	{
		AddTextLine(strText, "");
		AddTextLine(strText, GCStrings::getString(17).c_str());
		AddTextLine(strText, "");
	...
	AddTextLine(strText, "");
	AddTextLine(strText, GCStrings::getString(24).c_str());
	AddTextLine(strText, "");
...
//	nMin = int((vc.shour*24 - nHour)*60);
	...
	str.Format("\\par\\pard{\\f2\\fs%d %s}", g_HeaderSize, GCStrings::getString(25));
	strText += str;
	...
	{
		AddTextLineRtf(strText, "");
		str.Format("{\\fs%d %s}", g_Header2Size, GCStrings::getString(17).c_str());
		AddTextLineRtf(strText, str);
		AddTextLineRtf(strText, "");
	...
	AddTextLineRtf(strText, "");
	str.Format("{\\fs%d %s}", g_Header2Size, GCStrings::getString(24).c_str());
	AddTextLineRtf(strText, str);
	AddTextLineRtf(strText, "");
...
int FormatMasaListRtf(TResultMasaList &mlist, TString &str)
{
	...
	stt.Format("{\\fs%d\\f2 %s}\\par\\tx%d\\tx%d\\f2\\fs%d\r\n\\par\r\n%s: %s\\par\r\n"
		, g_HeaderSize
		, GCStrings::getString(39).c_str(), 1000*g_TextSize/24, 4000*g_TextSize/24
		, g_TextSize, GCStrings::getString(40).c_str(), mlist.m_location.m_strFullName.c_str());
	str += stt;
...
//				m_text += "\\par\r\n";
			...
			if (bShowColumnHeaders)
			{
				str.Format("\\par\\pard\\fs%d\\qc %s", g_NoteSize, GCStrings::GetVersionText());
				m_text += str;
				m_text += "\\par\\par\r\n";
			}
